...
with sync_playwright() as p:
 ...
 page.click('#reset');page.set_input_files('#load',str(save));page.wait_for_function("document.querySelector('#delivered').textContent==='1'")
 check('reload reconstructs incident',page.locator('#incident').is_visible())
 invalid=E/'bad.json';invalid.write_text('{bad');page.set_input_files('#load',str(invalid));page.wait_for_function("!document.querySelector('#error').hidden")
 check('invalid import is atomic',page.locator('#delivered').inner_text()=='1')
 page.set_input_files('#load',str(save));page.wait_for_function("document.querySelector('#error').hidden")
 for _ in range(3):page.click('#next')
 ...
 inject=E/'markup.json';inject.write_text(json.dumps(state));page.set_input_files('#load',str(inject));page.wait_for_function("document.querySelector('#note').textContent.includes('<img')")
