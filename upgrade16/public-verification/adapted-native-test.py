...
R=Path(__file__).resolve().parents[1];S=R.parent/'public/cutproof/v15';E=R/'public-verification';E.mkdir(exist_ok=True);checks=[];errors=[];requests=[]
report={'status':'running','tests':checks,'method':'actual Chromium and anonymous public HTTPS; failure injection is labelled','live_model_inference_rerun':False,'public_deployment':True}
...
base='https://6aa1e73893b44800087c7f1d--josephm.netlify.app/cutproof/v15/'
...
try:
 ...
 with sync_playwright()as pw:
  ...
  p.goto(base,wait_until='networkidle');expect(p.locator('#loadPassageExample')).to_be_visible();ok('Anonymous HTTPS launch displays the quickstart and original editor')
  ...
  assert all(u.startswith('https://6aa1e73893b44800087c7f1d--josephm.netlify.app/')or u.startswith('blob:')or u.startswith('data:')for u in requests);ok('The context-review task sends no media or inference requests to an external host')
  c.close();b.close()
