...
CASES = [
    {"id":"rice-correction","file":"rice-correction.pcm","spoken":"Rice twelve bags. No, thirteen bags.","expected":{"sku":"rice","quantity":13,"unit":"bags"}},
    {"id":"soap-number-correction","file":"soap-correction.pcm","spoken":"Soap fifteen bars. Sorry, fifty bars.","expected":{"sku":"soap","quantity":50,"unit":"bars"}},
    {"id":"oil-straight","file":"oil.pcm","spoken":"Cooking oil eight bottles.","expected":{"sku":"oil","quantity":8,"unit":"bottles"}},
    {"id":"wrong-unit","file":"wrong-unit.pcm","spoken":"Rice twelve cartons.","expected":None},
]
...
def run_case(case: dict, api_key: str) -> dict:
    ...
    with connect(url, open_timeout=20, close_timeout=10, max_size=2_000_000) as ws:
        # Receive Begin before streaming so we know the provider session identity.
        ...
        # Ask the service to flush/finalize. The result isn't complete until Termination.
    ...
    return {
        "id":case["id"],"spoken_fixture":case["spoken"],"expected":case["expected"],
        "audio_sha256":hashlib.sha256(audio).hexdigest(),"pcm_bytes":len(audio),
        "provider_session_id":first["id"],"events":safe,"final_turns":len(finals),
        "wall_seconds":round(termination_at-started,3),"flush_seconds":round(termination_at-sent_at,3),
    }
...
def main() -> int:
    ...
    rows=[]
    for case in CASES:
        rows.append(run_case(case,key))
    report={"schema":"recount-live-assemblyai-1","provider":"AssemblyAI Streaming v3","speech_model":"universal-3-5-pro","audio_source":"espeak-ng authored synthetic fixtures; not human speech","cases":rows,"provider_calls":len(rows),"human_audio_cases":0,"key_or_token_recorded":False}
