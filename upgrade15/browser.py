...
try:
 with sync_playwright()as p:
  ...
  caption('The app plays the surrounding source.\nSilent walkthrough; the source audio is synthetic.',6)
  ...
  subprocess.run(['ffmpeg','-v','error','-y','-i',str(video_path),'-vf',f"pad=1440:1120:0:0:color=0x101722,subtitles={srt}:force_style='Fontsize=8,PrimaryColour=&HFFFFFF,Outline=1,Alignment=2,MarginV=3'",'-c:v','libx264','-preset','fast','-crf','21','-pix_fmt','yuv420p','-an','-movflags','+faststart',str(O/'passage-demo.mp4')],check=True,timeout=180)
  subprocess.run(['ffmpeg','-v','error','-i',str(O/'passage-demo.mp4'),'-f','null','-'],check=True,timeout=90);ok('Actual-app delta walkthrough encoded and fully decoded; captioned, no simulated call or narration')
