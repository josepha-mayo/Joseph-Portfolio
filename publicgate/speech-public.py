...
ROOT=Path.cwd();OUT=ROOT/'public/cutproof/v15';EV=ROOT/'publicgate/results/speech';report={'status':'running','checks':[],'speech_results':[]}
def ok(name,**details):report['checks'].append({'name':name,**details});print('PASS',name,flush=True)
class Quiet(SimpleHTTPRequestHandler):
 def log_message(self,*args):pass
srv=ThreadingHTTPServer(('127.0.0.1',0),partial(Quiet,directory=str(OUT)));threading.Thread(target=srv.serve_forever,daemon=True).start();base='https://6aa1c2967587b50008d959c8--josephm.netlify.app/cutproof/v15/'
try:
 ...
 (EV/'browser.json').write_text(json.dumps(report,indent=2));srv.shutdown()
