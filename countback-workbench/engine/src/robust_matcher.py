...
def describe(gray, config):
    """Low-light feature preparation; no generative enhancement or scene labels.

    Global allocation can discard the small remote in favour of text on a book.
    A 4x4 spatial quota retains landmarks throughout the input. Points are returned
    in original image coordinates; original geometric acceptance gates apply.
    """
    scale=min(1.0, 1200.0/max(gray.shape))
    small=cv.resize(gray,None,fx=scale,fy=scale,interpolation=cv.INTER_AREA) if scale<1 else gray
    smooth=cv.GaussianBlur(small,(3,3),.6)
    work=cv.createCLAHE(clipLimit=2.0,tileGridSize=(8,8)).apply(smooth)
    detector=cv.SIFT_create(nfeatures=min(10000,config.features*3),contrastThreshold=.012,edgeThreshold=12)
    points=detector.detect(work,None)
    buckets=[[]for _ in range(16)]
    h,w=work.shape
    for p in points:
        cell=min(3,int(p.pt[1]*4/h))*4+min(3,int(p.pt[0]*4/w))
        buckets[cell].append(p)
    for b in buckets:b.sort(key=lambda p:(-p.response,p.pt[1],p.pt[0],p.angle))
    selected=[];j=0
    while len(selected)<config.features and any(j<len(b)for b in buckets):
        for b in buckets:
            if j<len(b) and len(selected)<config.features:selected.append(b[j])
        j+=1
    if not selected:return [],None
    kp,desc=detector.compute(work,selected)
    if desc is None:return [],None
    # Root-normalized SIFT: Hellinger distance via a Euclidean matcher.
    desc=np.sqrt(desc/(desc.sum(axis=1,keepdims=True)+1e-12)).astype(np.float32)
    for p in kp:p.pt=(p.pt[0]/scale,p.pt[1]/scale);p.size/=scale
    return kp,desc
...
def inspect(reference: np.ndarray, view: np.ndarray, config: Config | None = None) -> dict[str, Any]:
    ...
    kp1, des1 = describe(ref, config)
    kp2, des2 = describe(scene, config)
