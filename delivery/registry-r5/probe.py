"""Authored small OCI fixture: private publishing and actual anonymous Docker pull."""
from __future__ import annotations
import gzip,hashlib,io,json,pathlib,subprocess,tarfile,tempfile,time
import boto3,requests
REGION='us-east-1';STACK='von-registry-r5b-20260925'
OCI='application/vnd.oci.image.manifest.v1+json';CONFIG='application/vnd.oci.image.config.v1+json';LAYER='application/vnd.oci.image.layer.v1.tar+gzip'
def digest(data):return 'sha256:'+hashlib.sha256(data).hexdigest()
def encoded(obj):return json.dumps(obj,separators=(',',':'),sort_keys=True).encode()
def main():
 started=time.monotonic();state=boto3.client('cloudformation',region_name=REGION).describe_stacks(StackName=STACK)['Stacks'][0]
 ...
 base,repo,bucket=out['RegistryUrl'].rstrip('/'),out['Repository'],out['Bucket'];host=base.removeprefix('https://');assert host.endswith('.cloudfront.net')
 ...
 with tarfile.open(fileobj=stream,mode='w',format=tarfile.USTAR_FORMAT) as tar:
  info=tarfile.TarInfo('proof.txt');info.size=len(proof);info.mode=0o644;info.mtime=0;tar.addfile(info,io.BytesIO(proof))
 ...
 for payload,kind,ref,media in [(layer,'blobs',digest(layer),LAYER),(config,'blobs',digest(config),CONFIG),(manifest,'manifests',digest(manifest),OCI),(manifest,'manifests','smoke',OCI)]:
  assert len(payload)<16384;s3.put_object(Bucket=bucket,Key=f'v2/{repo}/{kind}/{ref}',Body=payload,ContentType=media,Metadata={'digest':digest(payload)})
 ...
 body=b'no-write';integrity={'x-amz-content-sha256':hashlib.sha256(body).hexdigest()};result['public_write_statuses']={}
 for method in ('PUT','POST','DELETE','PATCH'):
  reply=session.request(method,url,data=body,headers=integrity,timeout=10);result['public_write_statuses'][method]=reply.status_code
  # OAC may reject a body at 403 before the read-only handler rejects it at 405.
  check('deny_'+method.lower(),reply.status_code in (403,405))
 check('manifest_unchanged_after_denied_writes',session.get(url,timeout=10).content==manifest)
 check('unknown_repository',session.get(base+'/v2/not-our-repository/manifests/smoke',timeout=10).status_code==404)
 direct=f'https://{bucket}.s3.{REGION}.amazonaws.com/v2/{repo}/blobs/{digest(layer)}';check('direct_bucket_denied',session.get(direct,timeout=10).status_code==403)
 with tempfile.TemporaryDirectory(prefix='von-registry-probe-') as tmp:
  ...
  pulled=subprocess.run(['docker','--config',str(auth),'pull',ref],capture_output=True,text=True,timeout=120);result['docker_exit']=pulled.returncode
  check('real_docker_anonymous_pull',pulled.returncode==0)
  inspected=subprocess.run(['docker','image','inspect',ref],capture_output=True,text=True,check=True,timeout=10);data=json.loads(inspected.stdout)[0]
  ...
  with tarfile.open(archive) as image:
   ...
   if layer_content.startswith(b'\x1f\x8b'):layer_content=gzip.decompress(layer_content)
