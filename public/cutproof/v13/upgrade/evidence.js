...
const sensitive=t=>/^(?:no|not|never|without|only|unless|must|may|might|always|all|none|percent)$/.test(t)||/^\d/.test(t);
function compare(reference,observed){
 const a=words(reference),b=words(observed);assert(a.length<=800&&b.length<=800,'Speech comparison is limited to 800 words per side.');
