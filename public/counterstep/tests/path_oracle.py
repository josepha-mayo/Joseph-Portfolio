"""Independent AST/Fraction checks for task generation and requested-form rejection."""
from pathlib import Path
from fractions import Fraction as F
import ast,re,json,subprocess
R=Path(__file__).resolve().parents[1]
# This parser does not call or translate the JavaScript parser.
def affine(text):
...
for row in rows:
 ...
 for i,(a,out) in enumerate(zip(row['answers'],row['results'])):
  ...
  checks+=1
report={'status':'passed','generated_tasks':len(rows),'response_checks':checks,'independent_parser':'Python AST plus Fraction affine algebra; no eval and no JavaScript parser reuse','scope':'Synthetic authored task families, not learner results, new-model evaluation or a formal proof'}
