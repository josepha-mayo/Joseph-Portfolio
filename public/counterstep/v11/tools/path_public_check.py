...
def anchor_norm(s):
 def replace(m):
  ...
  if h in ['index.html','index','./','.']:attrs['href']='index.html'
  if h in ['path.html','path']:attrs['href']='path.html'
  if h in ['judge.html','judge']:attrs['href']='judge.html'
 ...
 with tempfile.TemporaryDirectory(prefix='counterstep-public-') as td:
  ...
  for name,meta in manifest.items():
   ...
   if not same:
    ...
    original=(R/name).read_text();served=data.decode();a=Blocks();b=Blocks();a.feed(original);b.feed(served);assert a.blocks==b.blocks,'Embedded executable/styles changed'
