...
import os,sys,json,threading,functools,http.server,socketserver,hashlib,time
...
try:
 with sync_playwright() as p:
  ...
  def run():
   page.click('#run');page.wait_for_function("document.querySelector('#run').disabled === false",timeout=30000)
  ...
  page.fill('#old','0,9,US,"<img src=x onerror=alert(1)>"');page.fill('#next','0,9,CN,Name');page.select_option('#family','4');page.fill('#traffic','');run();assert not page.locator('img').count();assert page.inner_text('#changed')=='10';ok('Untrusted country-name markup is not inserted into the document')
  page.click('#example');page.set_input_files('#oldFile',{'name':'input.csv','mimeType':'text/csv','buffer':(R/'examples/vendor-ipv4.csv').read_bytes()});page.wait_for_function("document.querySelector('#status').textContent.includes('Local file loaded')");run();assert page.inner_text('#changed')=='196,608';ok('Native file input feeds the actual comparison worker')
 ...
 import traceback
 report.update(status='failed',error=str(exc),traceback=traceback.format_exc(),checks=checks)
 raise
finally:
