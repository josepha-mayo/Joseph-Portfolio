...
const inputIds=['material','parts','remnants','new_stock','kerf','trim','reuse'];
function controls(){
...
function changed(){dirty=true;serial++;$('stale').hidden=!result;$('verified').textContent=result?'STALE / RECHECK':'NOT PLANNED';controls();}
...
function accept(data){
 ...
 $('verified').textContent=data.status==='optimal'?'EXACT PLAN / LEDGER PASSED':'SAVED CUTS REVALIDATED';
 ...
 $('message').textContent=data.solver?`Python examined ${data.solver.transitions.toLocaleString()} fitting transitions in ${data.solver.seconds}s. Optimal for this bounded model, not a physical cutting approval.`:'Cuts revalidated; solve again to prove the purchase objective.';
 const bars=[];
...
$('solve').onclick=()=>{try{send({action:'solve',job:readJob()},accept);}catch(e){error(e.message);}};
...
let examples;
$('example').onchange=()=>{fill(examples[$('example').value]);changed();error();};
